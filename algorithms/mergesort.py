...
def mergemax(l1: list[T], l2: list[T], key=lambda x: x) -> list[T]:
    """
    Merges two sorted lists into one larger sorted list,
    containing all elements from the smaller lists.
...
def mergesortmax(l: list[T], key=lambda x: x) -> list[T]:
    """
    Sort a list using the mergesort operation.
    :complexity: Best/Worst Case O(NlogN * comp(T))
    """
    if len(l) <= 1:
        return l
    break_index = (len(l)+1) // 2
    l1 = mergesortmax(l[:break_index], key=key)
    l2 = mergesortmax(l[break_index:], key=key)
    return mergemax(l1, l2, key=key)

def scoreMerge(l1,l2, adventurers):
    """
    Merges two sorted lists into one larger sorted list,
    containing all elements from the smaller lists.
    ...
    cur_right = 0
    while cur_left < len(l1) and cur_right < len(l2):
        if l1[cur_left].get_score(adventurers) >= l2[cur_right].get_score(adventurers):
            new_list.append(l1[cur_left])
            cur_left += 1
        else:
            new_list.append(l2[cur_right])
...
def scoreMergeSort(sites, adventurers):
    """
    Sort a list using the mergesort operation.
    :complexity: Best/Worst Case O(NlogN * comp(T))
    """
    ...
    l1 = scoreMergeSort(sites[:break_index], adventurers)
    l2 = scoreMergeSort(sites[break_index:], adventurers)
    return scoreMerge(l1, l2, adventurers)
